...
## Demo
[在线demo](https://mengshukeji.github.io/sheetDemo/)

![演示](/sheetDocs/img/sheetDemo.gif)
...
### 🔨操作体验
+ **撤销/重做**
+ **复制/粘贴/剪切操作** (支持sheet到excel和excel到sheet带格式的互相拷贝)
...
+ **EXCEL导入及导出** (专为sheet打造的导入导出插件，支持密码、水印、公式等的本地导入导出，导出正在开发)

### ⏱️未来开发计划
+ **打印及设置** (像excel一样进行打印设置，并导出为图片或者PDF)
+ **树形菜单** (类似excel中的分级显示（分组）)
+ **表格新功能** (类似excel中表格的筛选器和切片器)
+ **CSV,TXT导入及导出** (专为sheet打造的导入导出插件，支持密码、水印、公式等的本地导入导出)
...
注意，`https://cdn.jsdelivr.net/npm/sheet@latest/dist/sheet.umd.js`这个路径意思是会拉取到最新的sheet代码，但是如果sheet刚刚发布，jsdelivr网站可能还没来得及从npm上同步过去，故而使用这个路径还是会拉到上一个版本，我们推荐您直接指定最新版本。

想要指定sheet版本，请在所有的CDN依赖文件后面加上版本号，如：`https://cdn.jsdelivr.net/npm/sheet@2.1.12/dist/sheet.umd.js`。

> 如何知道最新版本是哪一版？查看最新 [release记录](https://github.com/mengshukeji/sheet/releases) 或者 [package.json](https://github.com/mengshukeji/sheet/blob/master/package.json) 的`version`字段。
...
一个完整的sheet表格文件的数据格式为：sheetfile，一个表格文件包含若干个sheet文件，对应excel的sheet0、sheet1等。

一个sheet文件的示例如下，该表格包含3个sheet：`
sheetfile = [ {sheet1设置},  {sheet2设置},  {sheet3设置} ]`
相当于excel的3个sheet

![excel sheet](/sheetDocs/img/excel.png)

文件中的一个sheet的数据`sheetfile[0]`的结构如下：
```json
{
	"name": "Cell", //工作表名称
	"color": "", //工作表颜色
...
如果您使用sheet遇到了问题，按以下步骤来寻找答案

1. 使用多吉或者Google搜索常见技术问题
2. sheet相关的问题优先查看[sheet官方文档](https://mengshukeji.github.io/sheetDocs/zh/)（注意标注TODO的功能暂未实现）
3. 搜索[常见问题列表](https://mengshukeji.github.io/sheetDocs/zh/guide/FAQ.html)
