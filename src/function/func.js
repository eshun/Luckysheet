    ...
    if(sp == "*"){ //乘
        if(getObjType(fp) == "array" && getObjType(tp) == "array"){
            ...
            if(getObjType(fp[0]) == "array" && getObjType(tp[0]) == "array"){
                    ...
                    for(let m = 0; m < fp.length; m++){
                        ...
                        for(let n = 0; n < fp[m].length; n++){
                            ...
                            if(isRealNum(fp[m][n]) && isRealNum(tp[m][n])){
                                value = sheet_calcADPMM(fp[m][n], sp, tp[m][n]);//parseFloat(fp[m][n]) * parseFloat(tp[m][n]);
                            }
                            else{
                                value = error.v;
                            }
                        ...
                        for(let n = 0; n < collen; n++){
                            ...
                            for(let p = 0; p < fp[0].length; p++){
                                ...
                                if(isRealNum(fp[m][p]) && isRealNum(tp[p][n])){
                                    value += sheet_calcADPMM(fp[m][p], sp, tp[p][n]);//parseFloat(fp[m][p]) * parseFloat(tp[p][n]);
                                }
                                else{
                                    value += error.v;
                                }
                        ...
                        for(let n = 0; n < collen; n++){
                            ...
                            for(let p = 0; p < tp[0].length; p++){
                                ...
                                if(isRealNum(tp[m][p]) && isRealNum(fp[p][n])){
                                    value += sheet_calcADPMM(fp[p][n], sp, tp[m][p]);//parseFloat(tp[m][p]) * parseFloat(fp[p][n]);
                                }
                                else{
                                    value += error.v;
                                }
                    ...
                    for(let m = 0; m < fp.length; m++){
                        ...
                        for(let n = 0; n < fp[m].length; n++){
                            ...
                            if(isRealNum(fp[m][n]) && isRealNum(tp[n])){
                                value = sheet_calcADPMM(fp[m][n], sp, tp[n]);//parseFloat(fp[m][n]) * parseFloat(tp[n]);
                            }
                            else{
                                value = error.v;
                            }
                    ...
                    for(let m = 0; m < rowlen; m++){
                        ...
                        for(let n = 0; n < collen; n++){
                            ...
                            if(isRealNum(fp[m][0]) && isRealNum(tp[n])){
                                value = sheet_calcADPMM(fp[m][0], sp, tp[n]);// parseFloat(fp[m][0]) * parseFloat(tp[n]);
                            }
                            else{
                                value = error.v;
                            }
                    ...
                    for(let m = 0; m < tp.length; m++){
                        ...
                        for(let n = 0; n < tp[m].length; n++){
                            ...
                            if(isRealNum(fp[n]) && isRealNum(tp[m][n])){
                                value = sheet_calcADPMM(fp[n], sp, tp[m][n]);// parseFloat(fp[n]) * parseFloat(tp[m][n]);
                            }
                            else{
                                value = error.v;
                            }
                    ...
                    for(let m = 0; m < rowlen; m++){
                        ...
                        for(let n = 0; n < collen; n++){
                            ...
                            if(isRealNum(fp[n]) && isRealNum(tp[m][0])){
                                value = sheet_calcADPMM(fp[n], sp, tp[m][0]);//parseFloat(fp[n]) * parseFloat(tp[m][0]);
                            }
                            else{
                                value = error.v;
                            }
                ...
                for(let n = 0; n < fp.length; n++){
                    ...
                    if(isRealNum(fp[n]) && isRealNum(tp[n])){
                        value = sheet_calcADPMM(fp[n], sp, tp[n]);// parseFloat(fp[n]) * parseFloat(tp[n]);
                    }
                    else{
                        value = error.v;
                    }
            ...
            if(getObjType(fp[0]) == "array"){
                for(let m = 0; m < fp.length; m++){
                    ...
                    for(let n = 0; n < fp[m].length; n++){
                        ...
                        if(isRealNum(fp[m][n]) && isRealNum(tp)){
                            value = sheet_calcADPMM(fp[m][n], sp, tp);// parseFloat(fp[m][n]) * parseFloat(tp);
                        }
                        else{
                            value = error.v;
                        }
                ...
                for(let n = 0; n < fp.length; n++){
                    ...
                    if(isRealNum(fp[n]) && isRealNum(tp)){
                        value = sheet_calcADPMM(fp[n], sp, tp);// parseFloat(fp[n]) * parseFloat(tp);
                    }
                    else{
                        value = error.v;
                    }
            ...
            if(getObjType(tp[0]) == "array"){
                for(let m = 0; m < tp.length; m++){
                    ...
                    for(let n = 0; n < tp[m].length; n++){
                        ...
                        if(isRealNum(fp) && isRealNum(tp[m][n])){
                            value = sheet_calcADPMM(fp, sp, tp[m][n]);// parseFloat(fp) * parseFloat(tp[m][n]);
                        }
                        else{
                            value = error.v;
                        }
                ...
                for(let n = 0; n < tp.length; n++){
                    ...
                    if(isRealNum(fp) && isRealNum(tp[n])){
                        value = sheet_calcADPMM(fp, sp, tp[n]);//parseFloat(fp) * parseFloat(tp[n]);
                    }
                    else{
                        value = error.v;
                    }
            ...
            if(isRealNum(fp) && isRealNum(tp)){
                result = sheet_calcADPMM(fp, sp, tp);//parseFloat(fp) * parseFloat(tp);
            }
            else{
                result = error.v;
            }
                        ...
                        for(let n = 0; n < collen; n++){
                            ...
                            for(let p = 0; p < fp[0].length; p++){
                                ...
                                if(isRealNum(fp[m][p]) && isRealNum(tp_inverse[p][n])){
                                    value += sheet_calcADPMM(fp[m][p], "*", tp_inverse[p][n]);// parseFloat(fp[m][p]) * parseFloat(tp_inverse[p][n]);
                                }
                                else{
                                    value += error.v;
                                }
        ...
        if(getObjType(fp) == "array" && getObjType(tp) == "array"){
                    ...
                    for(let n = 0; n < fp[m].length; n++){
                        ...
                        if(isRealNum(fp[m][n]) && isRealNum(tp[m][n])){
                            if(sp == "%" && parseFloat(tp[m][n]) == 0){
                                ...
                                value = sheet_calcADPMM(fp[m][n], sp, tp[m][n]);// eval(parseFloat(fp[m][n]) + sp + parseFloat(tp[m][n]));    
                            }
                        }
                        else{
                            value = error.v;
                    ...
                    for(let n = 0; n < fp[m].length; n++){
                        ...
                        if(isRealNum(fp[m][n]) && isRealNum(tp[n])){
                            if(sp == "%" && parseFloat(tp[n]) == 0){
                                ...
                                value = sheet_calcADPMM(fp[m][n], sp, tp[n]);//eval(parseFloat(fp[m][n]) + sp + parseFloat(tp[n]));    
                            }
                        }
                        else{
                            value = error.v;
                    ...
                    for(let n = 0; n < tp[m].length; n++){
                        ...
                        if(isRealNum(fp[n]) && isRealNum(tp[m][n])){
                            if(sp == "%" && parseFloat(tp[m][n]) == 0){
                                ...
                                value = sheet_calcADPMM(fp[n], sp, tp[m][n]);//eval(parseFloat(fp[n]) + sp + parseFloat(tp[m][n]));    
                            }
                        }
                        else{
                            value = error.v;
                ...
                for(let n = 0; n < fp.length; n++){
                    ...
                    if(isRealNum(fp[n]) && isRealNum(tp[n])){
                        if(sp == "%" && parseFloat(tp[n]) == 0){
                            ...
                            value = sheet_calcADPMM(fp[n], sp, tp[n]);//eval(parseFloat(fp[n]) + sp + "(" + parseFloat(tp[n]) + ")" );    
                        }
                    }
                    else{
                        value = error.v;
                    ...
                    for(let n = 0; n < fp[m].length; n++){
                        ...
                        if(isRealNum(fp[m][n]) && isRealNum(tp)){
                            if(sp == "%" && parseFloat(tp) == 0){
                                ...
                                value = sheet_calcADPMM(fp[m][n], sp, tp);//eval(parseFloat(fp[m][n]) + sp + parseFloat(tp));    
                            }
                        }
                        else{
                            value = error.v;
                ...
                for(let n = 0; n < fp.length; n++){
                    ...
                    if(isRealNum(fp[n]) && isRealNum(tp)){
                        if(sp == "%" && parseFloat(tp) == 0){
                            ...
                            value = sheet_calcADPMM(fp[n], sp, tp);//eval(parseFloat(fp[n]) + sp + parseFloat(tp));    
                        }
                    }
                    else{
                        value = error.v;
                    ...
                    for(let n = 0; n < tp[m].length; n++){
                        ...
                        if(isRealNum(fp) && isRealNum(tp[m][n])){
                            if(sp == "%" && parseFloat(tp[m][n]) == 0){
                                ...
                                value = sheet_calcADPMM(fp, sp, tp[m][n]);//eval(parseFloat(fp) + sp + parseFloat(tp[m][n]));    
                            }
                        }
                        else{
                            value = error.v;
                ...
                for(let n = 0; n < tp.length; n++){
                    ...
                    if(isRealNum(fp) && isRealNum(tp[n])){
                        if(sp == "%" && parseFloat(tp[n]) == 0){
                            ...
                            value = sheet_calcADPMM(fp, sp, tp[n]);//eval(parseFloat(fp) + sp + parseFloat(tp[n]));    
                        }
                    }
                    else{
                        value = error.v;
            ...
            if(isRealNum(fp) && isRealNum(tp)){
                if(sp == "%" && parseFloat(tp) == 0){
                    ...
                    result = sheet_calcADPMM(fp, sp, tp);//eval(parseFloat(fp) + sp + "(" + parseFloat(tp) + ")");    
                }
            }
            else{
                result = error.v;
...
function sheet_calcADPMM(fp, sp, tp){
    let value;
    if(sp=="+"){
...
function sheet_getcelldata(txt) {
    if (window.sheet_getcelldata_cache == null) {
        window.sheet_getcelldata_cache = {};
    }

    if (txt in window.sheet_getcelldata_cache) {
        return window.sheet_getcelldata_cache[txt];
    }
    ...
    if (val.length > 1) {
        ...
        sheetdata = sheetfile[index].data;
        rangetxt = val[0];
    ...
    if (rangetxt.indexOf(":") == -1) {
        ...
        if (!isNaN(row) && !isNaN(col)) {
            ...
            window.sheet_getcelldata_cache[txt] = retAll;
...
function sheet_parseData(value) {
...
function sheet_indirect_check_return(txt) {
    return txt;
}

function sheet_offset_check() {
...
function sheet_getSpecialReference(isCellFirst, param1, param2) {
    ...
    if(isCellFirst){
        return sheet_getcelldata(rangeTxt + ":" +functionRange.startCell);
    }
    else{
        ...
        return sheet_getcelldata(sheetName + functionRange.startCell + ":" + rangeT);
    }
...
export {
    ...
    sheet_parseData,
    sheet_getValue,
    sheet_indirect_check,
    sheet_indirect_check_return,
    sheet_offset_check,
    sheet_calcADPMM,
    sheet_getSpecialReference
}