...
function initialFilterHandler(){
    ...
    $(".sheet-cols-menu .sheet-cols-submenu").hover(
        ...
        function () {
            let _t = $(this), attrid = _t.attr("id"), _attr = $("#" + attrid + "_sub");
            submenuhide = setTimeout(function () { _attr.hide(); }, 200);
        }
    );
    ...
    $("#sheet-cell-main").on("click", ".sheet-filter-options", function (e) {
        ...
        let st_r = _t.data("str"), 
            ed_r = _t.data("edr"), 
            cindex = _t.data("cindex"), 
            st_c = _t.data("stc"), 
            ed_c = _t.data("edc"), 
        ...
        showrightclickmenu(_menu, toffset.left, toffset.top + 20);
    ...
    $(document).off("click.orderbyColorConfirm").on("click.orderbyColorConfirm", "#sheet-filter-orderby-color-submenu #sheet-filter-orderby-color-confirm", function(){
        ...
        $("#sheet-filter-orderby-color-submenu .item").each(function(i, e){
            if($(e).find("input[type='checkbox']").is(":checked")){
                ...
                if(_id == "filterBgColor"){
                    bg_colorMap[color] = 0;
                }
                else if(_id == "filterFcColor"){
                    fc_colorMap[color] = 0;
                }
            }
        });
            ...
            $("#sheet-filter-byvalue-select .ListBox input[type='checkbox']").each(function(i, e){
                if($(e).closest(".day").length > 0){
                    ...
                    if(itemV.indexOf(v) == -1){
                        ...
                        //天 对应的 月份
                        let _monthDay = $(e).closest(".dayList").find(".day:visible");
                        if(_monthDay.length == 0){
                            $(e).closest(".monthBox").find(".month").hide();
                        }

                        //天 对应的 年份
                        let _yearDay = $(e).closest(".monthList").find(".day:visible");
