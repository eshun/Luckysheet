...
function hideMenuByCancel(event){
    ...
    // Right-click the menu in the title bar, and click on the elements whose class is sheet-cols-rows-shift-left and sheet-cols-rows-shift-right will trigger the hiding of the menu bar. It should be prohibited. Exclude these two elements. There may be more Other elements will also jump here for more testing

    if(event.target.classList && (event.target.classList.contains('sheet-cols-rows-shift-left') || event.target.classList.contains('sheet-cols-rows-shift-right'))){
        return;
    }

    if (!$(event.target).hasClass("sheet-mousedown-cancel") && $(event.target).filter("[class*='sp-palette']").length == 0 && $(event.target).filter("[class*='sp-thumb']").length == 0 && $(event.target).filter("[class*='sp-']").length == 0) {
