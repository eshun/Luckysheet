...
function getRangetxt(sheetIndex, range, currentIndex) {
    ...
    if (sheetIndex != currentIndex) {
        ...
        sheettxt = Store.sheetfile[getSheetIndex(sheetIndex)].name.replace(/'/g,"''");
...
function getsheet_select_save() {
    return Store.sheet_select_save;
}

function getsheet_scroll_status() {
    return Store.sheet_scroll_status;
}

function getsheet_file(plugin) {
    // 获取图表数据
    if(plugin){
        Store.sheetfile.forEach(file => {
            if(!!file.chart){
